...
void RisalUI::_renderRoot(Print& out, const char* eff, bool rtl, int active) {
  ...
  if (strcmp(eff, "ru") == 0)
    out.print(F("window.RSL={set:'Настройки',lang:'Язык',theme:'Тема',accent:'Акцент',"
                 ...
                 "resetd:'Сброшено. Подключитесь к сети настройки, чтобы настроить заново.',update:'Обновить прошивку'};"));
    ...
    out.print(F("window.RSL={set:'Sozlamalar',lang:'Til',theme:'Mavzu',accent:'Urg\xCA\xBBu',"
                 ...
                 "resetd:'Tiklandi. Qayta sozlash uchun sozlash tarmog\xCA\xBBiga ulaning.',update:'Proshivkani yangilash'};"));
    ...
    out.print(F("window.RSL={set:'الإعدادات',lang:'اللغة',theme:'السمة',accent:'اللون',"
                 ...
                 "resetd:'تمت إعادة الضبط. اتصل بشبكة الإعداد للتهيئة من جديد.',update:'تحديث البرنامج'};"));
  out.print(F("window.RSACC="));
  out.print(_accent);
  out.print(F(";"));
  if (_ota) out.print(F("window.ROTA=1;"));  // Settings shows an "Update firmware" button (-> /update)
