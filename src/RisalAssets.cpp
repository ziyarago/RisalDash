// Core UI asset payloads (PROGMEM). Declared extern in RisalAssets.h.
#include "RisalUI.h"  // RISALDASH_VERSION for the footer string
#include "RisalAssets.h"

// Ends open at <title> — the renderer prints the device title, then RISAL_HEAD_END.
const char RISAL_HEAD[] PROGMEM =
  "<head><meta charset=\"UTF-8\">"
  "<meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">"
  "<meta name=\"theme-color\" content=\"#0F1115\"><title>";
const char RISAL_HEAD_END[] PROGMEM = "</title><style>";

const char RISAL_CSS[] PROGMEM =
  ...
  // effects(false) → .flat: drop the gradient wash + appbar blur, keep the palette.
  ".flat body,html.flat body{background-image:none}"
  ...
  // z-index scale: appbar(sticky) 30 · scrim 40 · drawer 50.
...
const char RISAL_STATUSBAR_JS[] PROGMEM = R"js((function(){
...
devHtml+(window.ROTA?'<div class="srow"><button class="supd" id="supd">'+L.update+'</button></div>':'')+'<div class="srow"><button class="sreset" id="sreset">'+L.reset+'</button></div><div class="snote">'+L.note+'</div></div>';
...
scrim.querySelectorAll('.gc').forEach(function(el){var k=el.dataset.k,t=el.dataset.t;
if(t==='toggle'){el.querySelectorAll('button').forEach(function(b){b.addEventListener('click',function(){el.querySelectorAll('button').forEach(function(x){x.classList.toggle('on',x===b);});fetch('/api/set?'+k+'='+b.dataset.v);});});}
else if(t==='select'){var cur=el.querySelector('.rsel-cur');
function op(o){el.classList.toggle('open',o);var r=el.parentNode;r.style.position=o?'relative':'';r.style.zIndex=o?'5':'';}
el.addEventListener('click',function(e){var o=e.target.closest('.rsel-opt');
...
var supd=scrim.querySelector('#supd');if(supd)supd.addEventListener('click',function(){location.href='/update';});
scrim.querySelector('#sreset').addEventListener('click',function(){if(confirm(L.resetc)){fetch('/api/forget',{method:'POST'}).catch(function(){});document.body.innerHTML='<div style="min-height:100vh;display:grid;place-items:center;text-align:center;padding:32px;font:600 16px var(--font,system-ui);color:#eaf0fb;background:#0b0f16">'+L.resetd+'</div>';}});
...
const char RISAL_LAYOUTS_JS[] PROGMEM = R"js((function(){
...
const char RISAL_APPBAR_END[] PROGMEM =
...
const char RISAL_INIT_JS[] PROGMEM =
...
const char RISAL_HTML_END[] PROGMEM = "</body></html>";

const char RISAL_PORTAL_CSS[] PROGMEM =
...
const char RISAL_PORTAL_JS[] PROGMEM = R"js((function(){
