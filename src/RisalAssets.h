...
static const char RISAL_CSS[] PROGMEM =
  ...
  "padding:56px clamp(14px,3vw,26px) 30px;background-attachment:fixed;background-image:"
  ...
  ".lay{display:none}.lay.on{display:grid}"
...
window.RSB={setTz:function(m){off=+m;tick();}};
var bat=+(window.RSB_BAT||82);
function drawBat(){var low=bat<=20;bar.querySelector(".rsb-pct").textContent=Math.round(bat);bar.querySelector(".rsb-fill").setAttribute("width",(27*bat/100).toFixed(1));bar.querySelector(".rsb-fill").setAttribute("fill",low?"#ff453a":"#34d399");bar.querySelector(".rsb-pct").setAttribute("stroke",low?"#3a0a0a":"#0a2a20");}
drawBat();setInterval(function(){bat+=(Math.random()<0.72?-1:1);if(bat>100)bat=100;if(bat<6)bat=6;drawBat();},9000);
})();)js";
...
var L=window.RSL||{set:'Settings',lang:'Language',theme:'Theme',accent:'Accent',note:'Saved · shown on every screen',dark:'Dark',light:'Light',auto:'Auto'};
...
var sheet=document.querySelector('.lsheet'),scrim=document.querySelector('.lscrim'),handle=document.querySelector('.lhandle');
if(!sheet)return;
...
function go(i){document.querySelectorAll('.lay').forEach(function(m){m.classList.toggle('on',+m.dataset.lay===i);});
  document.querySelectorAll('.ltile').forEach(function(t){t.classList.toggle('on',+t.dataset.lay===i);});
  handle.querySelector('span').textContent=nm(i);window.scrollTo(0,0);set(false);}
document.querySelectorAll('.ltile').forEach(function(t){t.addEventListener('click',function(){go(+t.dataset.lay);});});
var cur=document.querySelector('.ltile.on');handle.querySelector('span').textContent=cur?cur.querySelector('span').textContent:nm(0);
window.RL={open:function(o){set(!!o);},go:go};
var sy=0;
addEventListener('touchstart',function(e){sy=e.touches[0].clientY;},{passive:true});
addEventListener('touchend',function(e){var dy=sy-e.changedTouches[0].clientY;if(dy>45&&sy>innerHeight-100)set(true);},{passive:true});
})();)js";
