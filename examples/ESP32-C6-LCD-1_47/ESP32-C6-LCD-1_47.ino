...
// ── Author-string translations for the served dashboard ──────────────────────────────────────────
// The library localizes its own chrome (Settings, status bar). The strings WE write — page names,
// widget titles, sensor labels, select options — need dash.translate(). One row per source string
// (English) with its ru / uz / ar form; the whole comma-list is one row for a select's options.
...
const char *trWeb(const char *s, const char *lang) {
